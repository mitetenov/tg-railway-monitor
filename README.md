...
Бот позволяет через мастер настройки `/start` выбрать маршрут, дату и класс, после чего автоматически опрашивает API tkt.ge каждые 60 секунд и отправляет уведомление при появлении новых билетов или увеличении свободных мест. Поддерживает русский и английский интерфейс с автоопределением языка из профиля Telegram.
...
│
├── sample_stations.json         # 📊 Примеры данных от API tkt.ge
├── sample_rides.json
